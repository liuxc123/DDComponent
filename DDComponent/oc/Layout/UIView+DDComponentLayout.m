...
- (CGSize)sizeToFittingSize:(CGSize)targetSize layoutSize:(DDComponentLayoutSize *)layoutSize {
    ...
    CGSize effectiveSize = [self dd_effectiveSizeForContentSize:targetSize layoutSize:layoutSize];
    
    // Fixed width
    if (!layoutSize.widthDimension.isEstimated && layoutSize.heightDimension.isEstimated) {
        NSLayoutConstraint *temp = [NSLayoutConstraint constraintWithItem:self attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:targetSize.width];
    ...
    // Fixed height
    if (layoutSize.widthDimension.isEstimated && !layoutSize.heightDimension.isEstimated) {
        NSLayoutConstraint *temp = [NSLayoutConstraint constraintWithItem:self attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:targetSize.height];
        [self addConstraint:temp];
        effectiveSize = [self systemLayoutSizeFittingSize:UILayoutFittingCompressedSize];
        [self removeConstraint:temp];
    }
