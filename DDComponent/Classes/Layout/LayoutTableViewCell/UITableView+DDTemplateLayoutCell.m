...
@implementation UITableView (DDTemplateLayoutCell)

- (CGFloat)dd_systemFittingHeightForConfiguratedCell:(UITableViewCell *)cell {
    ...
    //
    // 1. Try "- systemLayoutSizeFittingSize:" first. (skip this step if 'dd_enforceFrameLayout' set to YES.)
    ...
    if (!cell.dd_enforceFrameLayout && contentViewWidth > 0) {
        ...
        [self dd_debugLog:[NSString stringWithFormat:@"calculate using system fitting size (AutoLayout) - %@", @(fittingHeight)]];
    }
    ...
    if (fittingHeight == 0) {
        ...
        if (cell.contentView.constraints.count > 0) {
            if (!objc_getAssociatedObject(self, _cmd)) {
                NSLog(@"[DDTemplateLayoutCell] Warning once only: Cannot get a proper cell height (now 0) from '- systemFittingSize:'(AutoLayout). You should check how constraints are built in cell, making it into 'self-sizing' cell.");
        ...
        [self dd_debugLog:[NSString stringWithFormat:@"calculate using sizeThatFits - %@", @(fittingHeight)]];
    }
...
- (CGFloat)dd_heightForCellWithIdentifier:(NSString *)identifier configuration:(void (^)(id cell))configuration {
    ...
    UITableViewCell *templateLayoutCell = [self dd_templateCellForReuseIdentifier:identifier];
    ...
    return [self dd_systemFittingHeightForConfiguratedCell:templateLayoutCell];
}
...
- (CGFloat)dd_heightForCellWithIdentifier:(NSString *)identifier cacheByIndexPath:(NSIndexPath *)indexPath configuration:(void (^)(id cell))configuration {
    ...
    // Hit cache
    if ([self.dd_indexPathHeightCache existsHeightAtIndexPath:indexPath]) {
        [self dd_debugLog:[NSString stringWithFormat:@"hit cache by index path[%@:%@] - %@", @(indexPath.section), @(indexPath.row), @([self.dd_indexPathHeightCache heightForIndexPath:indexPath])]];
        return [self.dd_indexPathHeightCache heightForIndexPath:indexPath];
    }
    
    CGFloat height = [self dd_heightForCellWithIdentifier:identifier configuration:configuration];
    [self.dd_indexPathHeightCache cacheHeight:height byIndexPath:indexPath];
    [self dd_debugLog:[NSString stringWithFormat: @"cached by index path[%@:%@] - %@", @(indexPath.section), @(indexPath.row), @(height)]];
...
- (CGFloat)dd_heightForCellWithIdentifier:(NSString *)identifier cacheByKey:(id<NSCopying>)key configuration:(void (^)(id cell))configuration {
    ...
    // Hit cache
    if ([self.dd_keyedHeightCache existsHeightForKey:key]) {
        CGFloat cachedHeight = [self.dd_keyedHeightCache heightForKey:key];
        [self dd_debugLog:[NSString stringWithFormat:@"hit cache by key[%@] - %@", key, @(cachedHeight)]];
        return cachedHeight;
    }
    
    CGFloat height = [self dd_heightForCellWithIdentifier:identifier configuration:configuration];
    [self.dd_keyedHeightCache cacheHeight:height byKey:key];
    [self dd_debugLog:[NSString stringWithFormat:@"cached by key[%@] - %@", key, @(height)]];
...
@implementation UITableView (DDTemplateLayoutHeaderFooterView)

- (__kindof UITableViewHeaderFooterView *)dd_templateHeaderFooterViewForReuseIdentifier:(NSString *)identifier {
    NSAssert(identifier.length > 0, @"Expect a valid identifier - %@", identifier);
...
- (CGFloat)dd_heightForHeaderFooterViewWithIdentifier:(NSString *)identifier configuration:(void (^)(id))configuration {
    UITableViewHeaderFooterView *templateHeaderFooterView = [self dd_templateHeaderFooterViewForReuseIdentifier:identifier];
...
@implementation UITableViewCell (DDTemplateLayoutCell)
